    ...
    <section className="mx-auto max-w-7xl px-4 py-10">
      ...
      <div className="grid md:grid-cols-3 gap-6">
        <div className="rounded-2xl border border-white/10 bg-white/5 p-6">
          ...
          <div className="text-3xl font-semibold text-emerald-300">${data.netWorthUSD.toLocaleString()}</div>
        </div>
            ...
            {data.tokens.map(t => (
              <div key={t.symbol} className="flex items-center justify-between text-white/90">
                <div className="flex items-center gap-3">
                  <div className="h-8 w-8 rounded-full bg-white/10 flex items-center justify-center font-bold text-xs">{t.symbol}</div>
